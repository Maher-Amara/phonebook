        ...
        case AJOUTER_POSITION_NOMBRE :
            {
                ...
                if (position > 1 && position < (taille+1)){
                    ajout_position = displayAjoutPositionFenetre(hwnd);
                }
...
void registerAjoutPositionNombreFenetre (HINSTANCE hInstance){
    ...
    ajoutPosNom.lpfnWndProc = AjoutPositionNombreFenetreProcedure;
...
HWND displayAjoutPositionNombreFenetre (HWND hwnd){
    ...
    box = CreateWindowW(L"Button", L" Position ",WS_CHILD | WS_VISIBLE | BS_GROUPBOX,(MainWidth)/4,(MainHeight)/8, (MainWidth)/2, (MainHeight)-200,ajout_position_nombre,NULL,NULL, NULL);
